...
FRP Manager 是基于 [frp](https://github.com/fatedier/frp) 的本地 GUI 内网穿透管理系统。服务端（frps + frp-server-agent）+ 客户端（Wails GUI + frpc）两部分，monorepo 结构。完整设计文档在 `docs/design.md`。
...
开始前建议先 `git log --oneline -20` 看完整提交历史，再读 `docs/design.md` 和 `docs/superpowers/plans/` 下的实现计划了解上下文。
